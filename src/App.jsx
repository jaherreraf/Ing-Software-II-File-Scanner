...
function App() {
  ...
  const scanFile = async () => {
    ...
    setIsScanning(true); // Indica que el escaneo ha comenzado
    ...
    const options = {
      headers: {
        'x-apikey': API_KEY,
        'Content-Type': 'multipart/form-data' // Axios maneja esto automáticamente, pero es buena práctica declararlo
      }
    };

    try {
      // 1. Enviar el archivo para escanear
      const response = await axios.post('https://www.virustotal.com/api/v3/files', formData, options);
      ...
      while (count < MAX_RETRIES) {
        ...
        analysisData = await analysisResponse.data;
      ...
      setIsScanning(false); // Indica que el escaneo ha terminado, ya sea con éxito o con error
    }
  };
  useEffect(() => {
  if (file) {
